...
describe "31179131 has_sage_flow_transitions adds methods to perform state transitions and save, e.g. do_lock!" do
  before(:each) do
    class Foo < Sample
  ...
  it "Changes the database when do_lock! does work" do
    f = Foo.create(name: "Jack", sage_flow_state: "saved")
    thisid = f.id
    f.do_lock!
    g = Foo.find(thisid)
    g.sage_flow_state.to_s.should == "locked"
  end
